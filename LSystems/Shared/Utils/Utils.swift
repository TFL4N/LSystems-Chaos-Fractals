...
extension CGColor {
    ...
    func toMTLColor() -> [Float] {
        let rgb_color = self.converted(to: CGColorSpaceCreateDeviceRGB(), intent: .absoluteColorimetric, options: nil)
        return rgb_color!.components!.map {Float($0)}
    }
